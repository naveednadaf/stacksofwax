...
app.get('/', (req, res) => {
    ...
    let api_url = `https://api.discogs.com/database/search?sort=want&page=1&per_page=50&type=release&q&format=album&location=South%2BKorea&language_exact=English&key=GTeeArlnsFjvomSGDmJe&secret=rVHVTGnuSyZeeFruGwaoboUuTuraknKg`
    let charts_title = [];
    let top_charts = [];
        ...
        results.forEach((item) => {
            ...
            `
                ...
                function handleresult(id)  {
                    let album_url = `https://api.discogs.com/releases/${id}?key=GTeeArlnsFjvomSGDmJe&secret=rVHVTGnuSyZeeFruGwaoboUuTuraknKg`;
    ...
    let api_most_collected = "https://api.discogs.com/database/search?sort=have%20desc&per_page=25&type=release&key=GTeeArlnsFjvomSGDmJe&secret=rVHVTGnuSyZeeFruGwaoboUuTuraknKg"
...
app.get('/library', (req, res) => {
    ...
    connection.query(query,[user_id], (err, results) => {
        ...
        promises = playlists.map((item) => {
            ...
            const album_url = `https://api.discogs.com/releases/${albumId}?key=GTeeArlnsFjvomSGDmJe&secret=rVHVTGnuSyZeeFruGwaoboUuTuraknKg`;
...
app.post('/delete', (req, res) => {
    ...
    let delete_query = `DELETE FROM album_playlist WHERE playlist_id = ${playlist_id} and album_id = ${album_id}`
    let search_query = `SELECT * FROM album_playlist WHERE playlist_id = ${playlist_id}`
    let playlist_query = `DELETE FROM playlist WHERE playlist_id = ${playlist_id}`
    connection.query(search_query,(err,results)=>{
        if(results.length === 1 ){
            connection.query(delete_query , (err)=>{
                if (err) throw err;
                connection.query(playlist_query, (err) => {
                    if (err) throw err;
...
app.get('/search' , (req, res)=>{
    ...
    let api_link = `https://api.discogs.com/database/search?q=${convertedText}&type=release&per_page=25&page=1&language_exact=English${formatQuery}${genreQuery}${yearQuery}&key=GTeeArlnsFjvomSGDmJe&secret=rVHVTGnuSyZeeFruGwaoboUuTuraknKg`;  
